...
for i in `cat ~/cram_file_paths.txt`;
	...
	echo '#$ -pe pvm 32'"\n\n" >> $HOME/cram2bam_jobs/${base_name}.qsub
	echo 'module load /mnt/storage/spack/share/spack/modules/linux-ubuntu16.04-x86_64/samtools-1.9-gcc-5.4.0-jjq5nua'"\n" >> $HOME/cram2bam_jobs/${base_name}.qsub
	echo "samtools view -T /mnt/storage/labs/sviswanathan/GATK_ref/Homo_sapiens_assembly38.fasta -b -o ${i}.bam ${i}.cram" >> $HOME/cram2bam_jobs/${base_name}.qsub
	done
